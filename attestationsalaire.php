...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
    ...
    <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
            <li class="nav-item">
                <a class="nav-link" href="attestationsalaire.php" style="color: #fff !important;font-size" >
...
<th class="sortable-header">Recevoir par</th>
...
<th>Action</th>
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM `wp_tablesome_table_6980` ORDER BY `id` DESC LIMIT $offset, $records_per_page";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>";
                if ($row['column_6'] == 'Recevoir la fiche par mail') {
        ...
        $total_records = mysqli_num_rows($conn->query("SELECT * FROM `wp_tablesome_table_6980`"));
...
<script> 
 $(document).ready(function() {
  ...
  function exportToExcel() {
    ...
    link.download = "attestationsalaire.xls";
            ...
            if (isConfirmed) {
                if (column_3.trim() !== "") {
                    customConfirm("Voulez-vous envoyer un SMS pour cette demande ?", function (sendSMS) {
                        ...
                        var url = `rhprescript.php?id=${id_demande}&traiteparColumn=column_12&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6980&update=column_11&smstext=${encodeURIComponent(smsText)}`;
                    ...
                    window.location.href = `rhprescript.php?id=${id_demande}&traiteparColumn=column_12&userColumn=${loggedInUserFullName}&update=column_11&table=wp_tablesome_table_6980&smstext=${encodeURIComponent(smsText)}`;
                }
            }
        });
    } else {
...
function confirmRejectStatus(id_demande, column_6, column_15) {
    ...
    if (storedUser) {
        ...
        customConfirm("Êtes-vous sûr de vouloir marquer cette demande comme étant Rejeté ?", function (isConfirmed) {
            if (isConfirmed) {
                var url = `reject.php?id=${id_demande}&traiteparColumn=column_12&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6980&update=column_11`;
                window.location.href = url; 
            } else {
