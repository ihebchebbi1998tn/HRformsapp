...
document.addEventListener("DOMContentLoaded", function() {
    ...
    <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item">
                <a class="nav-link" href="attestationstravail.php">
            ...
            <li class="nav-item">
                <a class="nav-link" href="fichedepaie.php"  style="color: #fff !important;font-size">
                    Fiches de paie
                    <?php
                        $countFiches = getCount($conn, 'column_11');
                        if ($countFiches > 0) {
  ...
  $('#etatSelect').on('change', function() {
    ...
    if (selectedEtat === 'Tout') {
      ...
      $('#dataTable tbody tr').each(function() {
        if ($(this).find('td:nth-child(10)').text().trim() === selectedEtat) {
    ...
    <thead class="thead-dark">
        <tr>
            ...
            <th class="sortable-header">Fiche du</th>
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM `wp_tablesome_table_6969` ORDER BY `id` DESC LIMIT $offset, $records_per_page";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>{$row['column_13']} <a href='javascript:void(0);' onclick='confirmSendSMS(\"{$row['column_13']}\", \"Votre fiche de paie est prête à être récupérée\")'><img src='Images/sendsms.png' alt='Icon' width='30' height='30'></a></td>";
                echo "<td>{$row['column_3']} - {$row['column_4']} </td>";
                echo "<td>";
                if ($row['column_7'] == 'Borne') {
                ...
                echo "</td>";
                echo "<td>";
                if ($row['column_14'] == 'Recevoir la fiche par mail') {
        ...
        $total_records = mysqli_num_rows($conn->query("SELECT * FROM `wp_tablesome_table_6969`"));
            ...
            if (isConfirmed) {
                if (column_3.trim() !== "") {
                    customConfirm("Voulez-vous envoyer un SMS pour cette demande ?", function (sendSMS) {
                        var smsText = "Votre fiche de paie est prête à être récupérée";

                        var url = `rhprescript.php?id=${id_demande}&traiteparColumn=column_15&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6969&update=column_6&smstext=${encodeURIComponent(smsText)}`;
                    ...
                    window.location.href = `rhprescript.php?id=${id_demande}&traiteparColumn=column_15&userColumn=${loggedInUserFullName}&update=column_6&table=wp_tablesome_table_6969&smstext=Votre fiche de paie est prête à être récupérée`;
                }
            }
        });
    } else {
...
function confirmRejectStatus(id_demande, column_6, column_15) {
    ...
    if (storedUser) {
        ...
        customConfirm("Êtes-vous sûr de vouloir marquer cette demande comme étant Rejeté ?", function (isConfirmed) {
            if (isConfirmed) {
                var url = `reject.php?id=${id_demande}&traiteparColumn=column_15&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6969&update=column_6`;
                window.location.href = url; 
            } else {
