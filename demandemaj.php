...
document.addEventListener("DOMContentLoaded", function() {
    ...
    <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item">
                <a class="nav-link" href="demandemaj.php" style="color: #fff !important;font-size" >
...
<br>
    ...
    <thead class="thead-dark">
        <tr>
            ...
            <th class="sortable-header">Mettre à jour</th>
            <th class="sortable-header">Avec</th>
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM `wp_tablesome_table_6925` ORDER BY `id` DESC LIMIT $offset, $records_per_page";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>{$row['column_4']}</td>";
                echo "<td>{$row['column_6']}</td>";
                echo "<td>";
                if ($row['column_10'] == 'Borne') {
                ...
                echo "<td>{$row['column_7']}</td>";
                echo "<td>{$row['column_13']}</td>";
                echo "<td>";
                if ($row['column_13'] === 'Nouveau') {
                  echo "<div class='button-container'>";
                  echo "<button class='btn btn-primary' onclick='confirmUpdateStatus(
                      {$row['id']},
                      \"{$row['column_3']}\",  // traité
                      \"{$row['column_13']}\" // type de demande
                  )'>Prête</button>";
                  echo "<button class='btn btn-danger reject-btn' onclick='confirmRejectStatus(
                    {$row['id']},
                    \"{$row['column_3']}\",  // column_6
                    \"{$row['column_13']}\" // column_14
                )'>X</button>";
                  echo "</div>";
              } else if  ($row['column_13'] === 'Rejetée')  {
        ...
        $total_records = mysqli_num_rows($conn->query("SELECT * FROM `wp_tablesome_table_6925`"));
            ...
            if (isConfirmed) {
                if (column_3.trim() !== "") {
                    customConfirm("Voulez-vous envoyer un SMS pour cette demande ?", function (sendSMS) {
                        var smsText = "Votre attestation de salaire est prête à être récupérée";

                        var url = `rhprescript.php?id=${id_demande}&traiteparColumn=column_7&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6925&update=column_13&smstext=${encodeURIComponent(smsText)}`;
                    ...
                    window.location.href = `rhprescript.php?id=${id_demande}&traiteparColumn=column_7&userColumn=${loggedInUserFullName}&update=column_13&table=wp_tablesome_table_6925&smstext=Votre attestation de salaire est prête à être récupérée`;
                }
            }
        });
    } else {
...
function confirmRejectStatus(id_demande, column_6, column_15) {
    ...
    if (storedUser) {
        ...
        customConfirm("Êtes-vous sûr de vouloir marquer cette demande comme étant Rejeté ?", function (isConfirmed) {
            if (isConfirmed) {
                var url = `reject.php?id=${id_demande}&traiteparColumn=column_7&userColumn=${loggedInUserFullName}&table=wp_tablesome_table_6925&update=column_13`;
                window.location.href = url; 
            } else {
